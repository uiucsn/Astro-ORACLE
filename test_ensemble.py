...
def parse_args():
    ...
    parser.add_argument('--num_input_models', type=int, required=True, help='Number of input models for the ensemble.')
...
def test_model(model_dir, num_input_models, test_dir=default_test_dir, max_class_count=default_max_class_count):
    ...
    models = []
    for i in range(num_input_models):
        models.append(keras.models.load_model(f"{model_dir}/input_model_{i}.h5", compile=False))
    ...
    for f in fractions:
        ...
        combined_input = []
        for m in models:
            combined_input.append(m.predict([x1, x2], batch_size=default_batch_size))

        y_pred = best_model.predict(combined_input, batch_size=default_batch_size)
