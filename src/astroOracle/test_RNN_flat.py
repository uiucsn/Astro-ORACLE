...
def run_day_wise_analysis(model, tree, model_dir, X_ts, X_static, Y, astrophysical_classes):
    ...
    for d in days:
        ...
        # Get the conditional probabilities
        flat_probs = np.copy(tf.nn.softmax(y_pred, axis=-1))
        pseudo_conditional_probabilities = np.zeros((flat_probs.shape[0], len(level_order_nodes)))

        pseudo_conditional_probabilities[:, -19:] = flat_probs
        ...
        # for parent, mask in zip(unique_parents, masks):
        #     print(parent, mask)
        level_order_nodes = np.asarray(level_order_nodes)

        pseudo_conditional_probabilities = pd.DataFrame(pseudo_conditional_probabilities, columns=level_order_nodes)

        for k in ['SN','Fast','Long','Periodic','Transient','Variable', 'Alert']:
            pseudo_conditional_probabilities[k] = np.sum(pseudo_conditional_probabilities.to_numpy()[:, masks[unique_parents.index(k)]], axis=1)

        pseudo_conditional_probabilities = pseudo_conditional_probabilities.to_numpy()
