...
from astroOracle.RNN_model import get_RNN_model, get_RNN_model_no_MD
from astroOracle.dataloader import load, get_augmented_data, get_static_features
from astroOracle.loss import weighted_categorical_crossentropy
...
default_model_dir = Path("models/flat")
...
def train_model(num_epochs=default_num_epochs, batch_size=default_batch_size, learning_rate=default_learning_rate, latent_size=default_latent_size, alpha=default_alpha, max_class_count=default_max_class_count, train_dir=default_train_dir, model_dir=default_model_dir, no_md=default_no_md):
    ...
    for c in np.unique(astrophysical_classes):
        ...
        Y_balanced += [Y[i][-19:] for i in idx] # This is done to enable flat classification
    ...
    level_order_nodes = list(nx.bfs_tree(tree, source=source_node_label).nodes())[-19:]
    level_order_counts = []

    # Find the weights
    for node in level_order_nodes:
        idx = np.where(np.asarray(a) == node)[0][0]
        level_order_counts.append(b[idx])
    level_order_counts = np.array(level_order_counts)

    weights = np.sum(level_order_counts)/ (level_order_counts * 19)
    print(level_order_nodes, weights)
    ...
    criterion = weighted_categorical_crossentropy(weights)
